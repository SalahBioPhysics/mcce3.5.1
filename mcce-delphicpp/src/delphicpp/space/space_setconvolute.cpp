...
//							}
			...
			for (j = 1; j <= iGrid; j++)
			{
				for (k = 1; k <= iGrid; k++)
				{
					if (ginit_rhomap[i][j][k] > rho_hs) {
						HRhomap[i][j][k] = 0.0;
						idebmap[i][j][k] = false;	//defining a new bDebmap to emulate the Gaussian module
					} else {
						HRhomap[i][j][k] = 1.0;
						idebmap[i][j][k] = true;
					}
		...
		if (conv_rhomap)
		{
			std::string convFileName = "conv_rhomap" + boost::lexical_cast<string>(inhomo) + ".dat";
			//std::string convFileName = "conv_rhomap.dat";
  ...
  if (inhomo == 0 && bEpsOut)
    ...
    //
    //
    // cout << " Conv> Epsilon Map output is requested." << endl;
    // cout << " Conv> Writing epsmap to " << strEpsFile << endl;
    //
		// ofstream eps_out(strEpsFile.c_str());
		// eps_out << "qdiffxs4 with an improved surfacing routine" << endl;
		// eps_out << "Gaussian cube format phimap" << endl;
    //
		// eps_out << fixed << setprecision(6);
		// eps_out << setw(5) << 1 << setw(12) << origin.nX << setw(12) << origin.nY << setw(12) << origin.nZ << endl;
		// eps_out << setw(5) << iGrid << setw(12) << stepsize/coeff << setw(12) << 0.0 << setw(12) << 0.0 << endl;
		// eps_out << setw(5) << iGrid << setw(12) << 0.0 << setw(12) << stepsize/coeff << setw(12) << 0.0 << endl;
		// eps_out << setw(5) << iGrid << setw(12) << 0.0 << setw(12) << 0.0 << setw(12) << stepsize/coeff << endl;
		// eps_out << setw(5) << 1 << setw(12) << 0.0 << setw(12) << 0.0 << setw(12) << 0.0 << setw(12) << 0.0 << endl;
    //
		// eps_out << setprecision(5);
    //
		...
		// 		{
		// 			eps_out << setw(13) << scientific << cepsmap[ix][iy][iz] << " " ;
		// 			if(iz%6 == 0) eps_out << endl;
		// 		}
		// 		eps_out << endl;
		// 	}
		// }
