...
time_t my_nowA, my_nowB, my_nowStart, my_nowEnd;
long    postrun_group_idum;
...
int rotamers()
{
   ...
   my_nowStart = time(NULL);
   if (env.test_seed < 0) postrun_group_idum = time(NULL); //allows random numbers to be fixed for testing
   else postrun_group_idum = env.test_seed;
   ...
   */
   ...
   if (env.rebuild_sc) {
       ...
       rebuild_sc(prot);
       my_nowB= time(NULL);
       printf("   Done. Time spent = %d\n\n", (int)(my_nowB - my_nowA)); fflush(stdout);
   }
   ...
   delete_h(prot);
   rm_dupconf_hv(prot);
   my_nowB= time(NULL); i = my_nowB - my_nowA;
   printf("   Done. Time spent = %d\n\n", i); fflush(stdout);
   ...
   for (j=1; j<n_conf; j++) { /* apply the rotation on passed in confs */
       ...
       * atoms of atom2, but not necessary in this conformer or this residue */
       ...
       if (strstr(rule.affected, "ALL_CONNECTED")) {
           int i_connect;
           n_connected = 1;
           connected = (ATOM**) malloc(sizeof(ATOM*));
           connected[0] = atom2_p;
           ...
           for (i_connect=0; i_connect<n_connected; i_connect++) {
               ...
               for (j_connect=0; j_connect<MAX_CONNECTED; j_connect++) {
                   ...
                   connected = (ATOM**) realloc(connected, n_connected*sizeof(ATOM *));
...
int rot_pack(PROT prot, int n)
{
    ...
    */
    ...
    pairwise = (float****) calloc(prot.n_res, sizeof(void *));
    /* initialize ngh list */
    for (i_res = 0; i_res < prot.n_res; i_res++) {
    ...
    for (i_res=0; i_res<prot.n_res; i_res++) {
        ...
        for (j_res=0; j_res<i_res; j_res++) {
            ...
            for (j_ngh=0; j_ngh<prot.res[j_res].n_ngh; j_ngh++) {
                if (i_res == prot.res[j_res].ngh[j_ngh]->i_res_prot) {
                    ...
                    prot.res[i_res].ngh = (RES_STRUCT**) realloc(prot.res[i_res].ngh, n_ngh*sizeof(void *));
                    prot.res[i_res].ngh[n_ngh-1] = &prot.res[j_res];
                    /* pointing pairwise(i,j) to pairwise(j,i), which would be transposed for i_res */
                    pairwise[i_res] = (float***) realloc(pairwise[i_res], n_ngh*sizeof(void *));
        ...
        for (j_res=i_res+1; j_res<prot.n_res; j_res++) {
            ...
            pairwise[i_res] = (float***) realloc(pairwise[i_res], n_ngh*sizeof(void *));
            pairwise[i_res][n_ngh-1] = (float**) malloc(prot.res[i_res].n_conf * sizeof(void *));
            if (!pairwise[i_res][n_ngh-1]) {printf("Memory Error\n"); return USERERR;}
            for (i_conf=1; i_conf<prot.res[i_res].n_conf; i_conf++) {
                pairwise[i_res][n_ngh-1][i_conf] = (float*) malloc(prot.res[j_res].n_conf * sizeof(float));
...
int rot_refine(PROT prot, MICROSTATE state, float ****pairwise) {
    ...
    for (i=0; i<prot.n_res; i++) {                  /* loop over residues */
        ...
        if (switching) {
            ...
            i_conf = candidate[(int)(ran2(&postrun_group_idum) * (float)n_candidate)];
...
int prune_pv(PROT prot, float c1, float c2, float c3)
{
   ...
   for (i_res = 0; i_res < prot.n_res; i_res++) {
       ...
       for (j_res = 0; j_res< prot.n_res; j_res++) {
           ...
           prot.res[i_res].n_ngh++;
           prot.res[i_res].ngh = (RES_STRUCT**) realloc(prot.res[i_res].ngh, prot.res[i_res].n_ngh * sizeof(RES *));
      ...
      for (ic=1; ic<prot.res[ir].n_conf-1; ic++) {
         ...
         for (jc=ic+1; jc<prot.res[ir].n_conf; jc++) {
            ...
            if (prot.res[ir].conf[ic].history[2] == 'O' && prot.res[ir].conf[jc].history[2] == 'O') {
			    if (ran2(&postrun_group_idum) < 0.5) prot.res[ir].conf[ic].on = 0;
...
int rand_conf_prune(PROT prot)
{
    ...
    for (i_conf=0;i_conf<2000;i_conf++) ran2(&postrun_group_idum);
    ...
    for (i_res=0; i_res<prot.n_res; ++i_res) {
        ...
        while (prot.res[i_res].n_conf-1 > n_hv_conf_limit) {
            i_conf = 1 + ran2(&postrun_group_idum) * (float) (prot.res[i_res].n_conf-1);
