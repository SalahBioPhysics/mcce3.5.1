...
{   int i, j, kr, kc, counter;
    ...
    for (kr=0; kr<prot.n_res;kr++) {
       for (kc=1; kc<prot.res[kr].n_conf; kc++) {
        ...
        if (((counter-n_dummies+1) >= env.delphi_start && (counter-n_dummies+1) <= env.delphi_end)) {
    ...
    for (kc=0; kc<ematrix.n; kc++) {
        ...
        if (((kc-n_dummies+1) >= env.delphi_start && (kc-n_dummies+1) <= env.delphi_end)) { /* in current calculation */
           ematrix.conf[kc].on = 't';
