	...
	public:     
      ...
      CDelphiData(SMCCE * mcce_data,shared_ptr<CTimer> pTimer):pddm(new CDelphiDataMarshal(pTimer))
      {
         ...
         //pddm->gfOffCenter.nZ = mcce_data->center[2];
		     ...
		     // equivalent to Acenter(x,y,z) in the parameter file
         pddm->bIsAcent   = mcce_data->bAcent; // Added this on June 1st, 2017. by Salah Salah
         pddm->gfAcent.nX = mcce_data->center[0]; 
         pddm->gfAcent.nY = mcce_data->center[1]; 
         pddm->gfAcent.nZ = mcce_data->center[2];
...
*/
        ...
        // equivalent to energy(g,s,c) in the parameter file
        pddm->bGridEng = true; 
        pddm->bSolvEng = prime_data->bSolvEng; 
        pddm->bCoulombEng = false;
